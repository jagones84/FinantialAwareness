...
## 2026-08-05: TDD Fix Campaign (audit F1-F7 + agent parity + Goal Solver locus)

### Status: Completed

#### Completed

1. **Audit fixes (F1-F7)**: surplus button placement, QuickStart wiring, risk-prompt formulas,
   FETCH_PAGE/threshold docs, Pareto compare mode, Goal Solver (bisection + dialog + agent tool).
2. **Agent parity & grounding**: RUN_OPTIMIZATION uses the user's GA config + Pareto modes;
   RUN_SENSITIVITY; multi-agent context grounded in real engine results; multi-command execution;
   effective curves in GET_FINANCIAL_CONTEXT; opt-in real-LLM e2e (AGENT_HARNESS_E2E=1).
3. **Scheduled expenses live**: screens now go through the ViewModel (dead-code bug fixed);
   renamed to "Scheduled Expenses" (en/it/es).
4. **Erase**: confirmation dialog on "Erase Analysis Results" (clears all in-memory analysis +
   chart markers; persisted data untouched).
5. **Sensitivity**: metric = AVERAGE UTILITY (was fObjW); bonus-weight row removed;
   rate rows per +1pp; daily surplus per +100 EUR/month.
6. **Goal Solver = locus**: P1 sweep 0..100% (table with (P1, capital_i) couples), Apply installs
   the selected row's FULL plan; P3=0 floor semantics documented and test-locked (post-stop
   utility == threshold, capital eroded).
7. **Domain manuals**: .agent/README-goal-solver.md, README-sensitivity.md, README-gui-state-wiring.md.

#### Verification

- 122 unit tests / 0 failures / 1 opt-in skip; assembleDebug green.
- WORKFLOW.md parts 1-8; HANDOFF sections 0/0b/0c/0d/0e/0f.
