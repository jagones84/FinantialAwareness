    ...
    fun constructSystemPrompt(inputs: FinancialInput, specificExpenses: List<SpecificExpense>, surplusData: SurplusInput): String {
        return """
            ...
            **Tool Usage:**
            ...
            - `RUN_RETIREMENT_SOLVER {"stopWorkAge": X, "happinessThreshold": T}`: Inverse analysis. Computes the MINIMUM initial capital needed to stop working at age X (all JSON input overrides supported, e.g. `"pensioneMensileNetta": 0.0` for a pure capital-based plan) while keeping utility >= T at every age. The result holds for the goal plan (stop work at X, p2 = p4 = X, p3 = 0, threshold = T); applying it installs the WHOLE plan, not just the capital. The optimizer may reach a higher score with the same capital via a less conservative plan. Use it when the user asks questions like "how much do I need to accumulate to stop working with happiness 0.3?". If infeasible, the output reports the maximum achievable utility and why.
